...
do
    ...
    awk -v sample_name="$fastq_name" '/^>/ {print ">" sample_name "_" ++seq_id} !/^>/ {print}' ./mice_fasta/"$fastq_name".fa > ./mice_fasta/"$fastq_name"_renamed.fa
    
    # Reemplazamos el archivo original por el renombrado
    mv ./mice_fasta/"$fastq_name"_renamed.fa ./mice_fasta/"$fastq_name".fa
done
